...
using MQTTnet.Extensions.ManagedClient.Persistence;
...
namespace MQTTnet.Extensions.ManagedClient
{
    public sealed class ManagedMqttClient : Disposable, IManagedMqttClient
    {
        ...
        readonly DbCacheQueue _messageQueueDb;
        ...
        public ManagedMqttClient(IMqttClient mqttClient, IMqttNetLogger logger, IServiceScopeFactory scopeFactory)
        {
            ...
            _messageQueueDb = new DbCacheQueue(scopeFactory);
        }
        ...
        public int PendingApplicationMessagesCount => Options.UseDbCache? _messageQueueDb.Count : _messageQueue.Count;
        ...
        public async Task EnqueueAsync(ManagedMqttApplicationMessage applicationMessage)
        {
            ...
            try
            {
                using (await _messageQueueLock.EnterAsync().ConfigureAwait(false))
                {
                    ...
                    {
                        _messageQueueDb.Enqueue(applicationMessage);
                    }
                    else
                    {
                        #region 超出缓存消息数量限制时，根据策略处理
        ...
        public async Task StartAsync(ManagedMqttClientOptions options)
        {
            ...
            if (options.Storage != null)
            {
                ...
                {
                    if (Options.UseDbCache)
                        _messageQueueDb.Enqueue(message);
                    else
        ...
        public async Task StopAsync(bool cleanDisconnect = true)
        {
            ...
            if (!Options.UseDbCache) _messageQueue.Clear();
        ...
        async Task PublishQueuedMessagesAsync(CancellationToken cancellationToken)
        {
            try
            {
                while (!cancellationToken.IsCancellationRequested && InternalClient.IsConnected)
                {
                    ...
                    var message = Options.UseDbCache ? _messageQueueDb.PeekAndWait(cancellationToken) : _messageQueue.PeekAndWait(cancellationToken);
                    if (message == null)
                    {
                        continue;
                    }
        ...
        async Task TryPublishQueuedMessageAsync(ManagedMqttApplicationMessage message, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                using (await _messageQueueLock.EnterAsync().ConfigureAwait(false)) //lock to avoid conflict with this.PublishAsync
                {
                    ...
                    if (Options.UseDbCache)
                        _messageQueueDb.RemoveFirst(message.Id);
                    else
            ...
            catch (MqttCommunicationException exception)
            {
                ...
                if (Options.UseDbCache)
                {
                    _messageQueueDb.DbLog(message.Id, exception.Message);
                }
                else
                {
                   #region 当消息级别为 QoS 0（最多一次）时，消息发送失败后，会从队列中移除
            ...
            catch (Exception exception)
            {
                ...
                if (Options.UseDbCache) _messageQueueDb.DbLog(message.Id, exception.Message);
            }
            finally
            {
